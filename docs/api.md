...
> `year`/`month`를 생략하면 오늘 날짜 기준 **이번 달**로 집계된다(다른 리포트 엔드포인트와 동일).
...
> `emotion_breakdown` vs `emotion_expense_ratio` — 둘 다 이번 달 기준이지만 분모가 다르다:
> - `emotion_breakdown`: **태그 부착 횟수** 기준 분포. 거래 하나에 태그가 여러 개 붙으면 그만큼
>   여러 태그에 나눠 집계되므로 5개 값의 **합이 항상 100%**(레이더 차트용)
> - `emotion_expense_ratio`: **해당 태그가 붙은 지출의 비율** — 이번 달 전체 지출 거래 수 대비
>   태그마다 독립적으로 계산. 한 거래에 여러 태그가 붙어도 각 태그가 자기 몫을 그대로
>   인정받으므로 5개 값의 **합이 100%를 넘거나 못 미칠 수 있음**(중복 집계 막대 그래프용)
